...
\end{align*}
%
We now observe that, under \lemref{gh_difference_from_one},
%
\begin{align*}
\MoveEqLeft
\sup_{w\in W_{\delta}}\norm{
    \hint\left(\thetaw, w\right) -
    H(\thetaone, \onevec)}_{2} \\
=& \sup_{w\in W_{\delta}}\norm{
    \int_0^1 \left(
        H(\thetaone + t (\theta - \thetaone), w) dt -
        H(\thetaone, \onevec)\right)}_{2} \\
\le&
    \sup_{w\in W_{\delta}}
    \int_0^1 \norm{
        H(\thetaone + t (\theta - \thetaone), w) -
        H(\thetaone, \onevec)}_{2} dt
        \textrm{ (Jensen's inequality)} \\
\le&
    \sup_{\theta\in\thetadeltaball} \sup_{w\in W_{\delta}}
    \norm{H(\theta), w) - H(\thetaone, \onevec)}_{2} \\
\le&
    \sup_{\theta\in\thetadeltaball} \sup_{w\in W_{\delta}}
    \norm{H(\theta, w) - H(\thetaone, \onevec)}_{2}
    \textrm{ (ordering of norms)}\\
\le& \left(1 + D\constw\liph\constop\right)\delta
